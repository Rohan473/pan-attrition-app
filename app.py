...
# Low-importance features to drop (noisy rate columns)
DROP_FEATURES = ["DailyRate", "HourlyRate", "MonthlyRate"]
...
def _data_fingerprint(X_scaled, y):
    """Hash the dataset so cache invalidates when data changes."""
    h = hashlib.md5(pd.util.hash_pandas_object(X_scaled).values.tobytes() +
                    pd.util.hash_pandas_object(y).values.tobytes()).hexdigest()
    return h
...
def train_models(X_scaled, y):
    ...
    for name, base_mdl in base_models.items():
        ...
        cv_scores = cross_val_score(best_mdl, X_res, y_res, cv=skf, scoring="roc_auc")

        y_pred = best_mdl.predict(X_test)
        y_prob = best_mdl.predict_proba(X_test)[:, 1]
        results[name] = {
            "model":       best_mdl,
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    st.dataframe(metrics_df.style.format({
        ...
        "F1-Score":"{:.3f}","ROC-AUC":"{:.3f}","CV Mean":"{:.3f}","CV Std":"{:.3f}",
    }).highlight_max(subset=["Accuracy","Precision","Recall","F1-Score","ROC-AUC","CV Mean"],
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    # Build a synthetic employee row using median/mode values as defaults
    median_row = df.median(numeric_only=True).to_dict()

    # Start with all features at 0 (important for one-hot columns)
    employee = {col: 0 for col in feature_names}
    # Fill numeric features with medians
    for col in feature_names:
        if col in median_row:
            employee[col] = median_row[col]

    # Override with user inputs (numeric features)
    override = {
        "Age": age,
    ...
    # Set one-hot encoded OverTime column
    if "OverTime_Yes" in feature_names:
        override["OverTime_Yes"] = 1 if overtime == "Yes" else 0
    employee.update({k: v for k, v in override.items() if k in feature_names})
    ...
    for feat in ["JobSatisfaction","MonthlyIncome","WorkLifeBalance",
        ...
        if feat in feature_names:
            ...
            for v in vals_range:
                tmp = emp_df.copy()
                tmp[feat] = v
                # Recalculate engineered features that depend on the swept feature
                js  = tmp["JobSatisfaction"].iloc[0]
                ji  = tmp["JobInvolvement"].iloc[0]
                es  = tmp["EnvironmentSatisfaction"].iloc[0]
                rs  = tmp["RelationshipSatisfaction"].iloc[0]
                mi  = tmp["MonthlyIncome"].iloc[0]
                twy = tmp["TotalWorkingYears"].iloc[0]
                ot_yes  = tmp["OverTime_Yes"].iloc[0]
                wl  = tmp["WorkLifeBalance"].iloc[0]
                ysp = tmp["YearsSinceLastPromotion"].iloc[0]
                tmp["EngagementScore"] = (js + ji + es + rs) / 4
                tmp["IncomePerExp"]    = mi / (twy + 1)
                tmp["PromotionDelay"]  = int(ysp > 3)
                tmp["WorkloadStress"]  = int(ot_yes == 1 and wl <= 2)
